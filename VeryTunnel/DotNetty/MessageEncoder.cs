...
namespace VeryTunnel.DotNetty
{
    ...
    public class MessageEncoder : MessageToByteEncoder<ChannelMessage>
    {
        ...
        private readonly static Lazy<Dictionary<Type, Action<WrappedMessage, IMessage>>> _setInnerMessageFuncs = new Lazy<Dictionary<Type, Action<WrappedMessage, IMessage>>>(() =>
        {
#if NETSTANDARD2_0 || NET472
            var properties = typeof(WrappedMessage).GetProperties().Where(x => x.PropertyType.IsSubclassOf(typeof(IMessage))).ToList();
#else
